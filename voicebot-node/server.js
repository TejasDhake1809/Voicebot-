...
import { initDB, InteractionLog, User } from "./services/db.js";
...
app.post("/api/register", async (req, res) => {
  try {
    ...
    if (!username || !password || !accountId) return res.status(400).json({ error: "username, password, accountId required" });
...
app.post("/api/login", async (req, res) => {
  try {
    ...
    const token = jwt.sign({ userId: user._id.toString(), accountId: user.accountId }, process.env.JWT_SECRET, { expiresIn: "4h" });
...
app.post("/api/message", authMiddleware, async (req, res) => {
  try {
    ...
    const replyObj = await generateResponse(text, nlu.intent, nlu.entities, req.user.accountId);
...
// VOICE BOT
app.post("/api/voice-bot", authMiddleware, upload.single("audio"), async (req, res) => {
  try {
    ...
    const replyObj = await generateResponse(inputText, nlu.intent, nlu.entities, req.user.accountId);
    const responseText = typeof replyObj === "string" ? replyObj : (replyObj?.text || "");
    const suggestSave = !(typeof replyObj === "string") && !!replyObj?.suggestSave;
...
app.post("/api/save-question", authMiddleware, async (req, res) => {
  try {
    ...
    if (!questionToSave) return res.status(400).json({ error: "No question found to save. Please re-state the question." });
